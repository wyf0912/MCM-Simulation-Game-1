...
\label{oval}	
\end{figure}
\indent When hot water is added at in $V_{best}$, the temperature will only change in a very small range. It means the temperature will nearly not change.\\
%温度下降和温度稳定的比较
...
\label{oval}	
\end{figure}
\indent The curves tell that our hot water is set perfectly, the temperature will neither get too high nor get too low. And the goal of keeping the temperature as close as possible to the initial temperature is achieved.
...
We compare two ways of adding hot water. And when  
\begin{figure}[H]
